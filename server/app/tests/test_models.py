...
from .models import Employee, Attendance, LeaveBalance, RecentActivity, LeaveRequest, TeamAttendance, TeamLeaveRequest, OrganizationDirectory, OrganizationStructure, EmployeeProfile, AttendanceReport, Notification, Role, EmployeeRole
...
class RoleModelTest(TestCase):
    def setUp(self):
        self.role = Role.objects.create(name='Admin', permissions=['can_edit', 'can_delete'])
    def test_role_creation(self):
        self.assertEqual(self.role.name, 'Admin')
        self.assertEqual(self.role.permissions, ['can_edit', 'can_delete'])
    def test_unique_role_name(self):
        with self.assertRaises(Exception):
            Role.objects.create(name='Admin', permissions=['can_view'])
class EmployeeRoleModelTest(TestCase):
    def setUp(self):
        self.employee = Employee.objects.create(employee_id='EMP001', name='John Doe')
        self.role = Role.objects.create(name='User', permissions=['can_view'])
        self.employee_role = EmployeeRole.objects.create(employee=self.employee, role=self.role)
    def test_employee_role_creation(self):
        self.assertEqual(self.employee_role.employee.employee_id, 'EMP001')
        self.assertEqual(self.employee_role.role.name, 'User')
    def test_employee_role_employee_relation(self):
        self.assertEqual(self.employee_role.employee.employee_id, 'EMP001')
